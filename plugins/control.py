...
"""
    ...
    return await app.send_message(
        ...
        MessageChain(
            Image(
                data_bytes=await create_md(
                    md, width=360, height=(md.count("\n") + 5) * 16
                )
            )
        ),
    )
...
async def _m_main(app: Ariadne, sender: Sender):
    return await app.send_message(
        sender, await AlconnaDispatcher.default_send_handler(module_control.get_help())
    )
...
async def _f_main(app: Ariadne, sender: Sender):
    return await app.send_message(
        sender,
        await AlconnaDispatcher.default_send_handler(function_control.get_help()),
    )
...
"""
    for i in bot.data.funcs:
        stat = (
            "❌ 禁用" if i in group.disabled else "🚫 黑名单" if group.in_blacklist else "✔ 启用"
        )
...
async def _g_main(app: Ariadne, sender: Sender):
    return await app.send_message(
        sender, await AlconnaDispatcher.default_send_handler(group_control.get_help())
    )
...
@assign("自检")
@alcommand(group_control, send_error=True)
async def _g_check(app: Ariadne, sender: Sender, bot: RaianMain):
...
@assign("列出")
@alcommand(group_control, send_error=True)
async def _g_list(app: Ariadne, sender: Sender, bot: RaianMain):
