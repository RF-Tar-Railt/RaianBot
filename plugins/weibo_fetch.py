...
async def add(app: Ariadne, group: Group, source: Source, result: AlconnaProperty):
    ...
    if int(follower.id) in followers:
        return await app.sendGroupMessage(group, MessageChain.create(f"该群已关注 {follower.name}！请不要重复关注"))
    followers.append(int(follower.id))
    profile.additional['weibo_followers'] = followers
    bot.data.update_group(profile)
    return await app.sendGroupMessage(group, MessageChain.create(f"关注 {follower.name} 成功！"), quote=source.id)
...
async def add(app: Ariadne, group: Group, source: Source, result: AlconnaProperty):
    ...
    if int(follower.id) not in followers:
        return await app.sendGroupMessage(group, MessageChain.create(f"该群未关注 {follower.name}！"))
    followers.remove(int(follower.id))
    profile.additional['weibo_followers'] = followers
    bot.data.update_group(profile)
    return await app.sendGroupMessage(group, MessageChain.create(f"解除关注 {follower.name} 成功！"), quote=source.id)
